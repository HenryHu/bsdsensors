...
#include "super_io.h"
#include "port_io.h"
#include "microchip_chip_info.h"

namespace bsdsensors {

using LogicalDeviceNo = uint8_t;

const uint32_t kMicrochipPorts[] = {0x2E, 0x4E, 0x162E, 0x164E};
...
class MicrochipLock {
   public:
    MicrochipLock(MicrochipChip* chip) : chip_(chip) { chip_->Enter(); }
    ~MicrochipLock() { chip_->Exit(); }
...
class MicrochipChipImpl : public MicrochipChip {
   public:
    ...
    Status Enter() override {
        entered_ = true;
        return io_->WriteByte(kConfigRegister, 0x55);
    }

    // Exit Extended Function mode
    Status Exit() override {
        entered_ = false;
        return io_->WriteByte(kConfigRegister, 0xAA);
    }
    ...
    bool Detect() override {
        ...
        for (const auto& port : kMicrochipPorts) {
...

            {
                ...
                if (id != 0xffff) {
                    std::cout << "Found Microchip chip, ID: " << std::hex
                              << "0x" << id << " at 0x" << port << std::endl;
                    ...
                    if (info_ != nullptr) {
                        ...
                        std::cout << "Unknown Microchip Chip" << std::endl;
                        return false;
                    }
                }
            }
